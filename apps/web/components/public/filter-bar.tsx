  ...
  return (
    <form className="filter-bar" action="/" aria-label="內容篩選">
      <div className="filter-bar__items">
        <label className="filter-field"><span>主題</span><select name="topic" defaultValue={topic}><option value="artificial-intelligence">所有 AI 主題</option>{topics.filter((item) => item.slug !== "artificial-intelligence").map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}</select></label>
