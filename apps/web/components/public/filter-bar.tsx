...
export function FilterBar({ topic, difficulty, sort, language, published, contentType, topics }: FilterBarProps) {
  return (
    <form className="filter-bar" action="/" aria-label="內容篩選">
      <div className="filter-bar__row filter-bar__row--primary">
        <label className="filter-field"><span>主題</span><select name="topic" defaultValue={topic} onChange={submitOnChange}><option value="artificial-intelligence">所有 AI 主題</option>{topics.filter((item) => item.slug !== "artificial-intelligence").map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}</select></label>
        <label className="filter-field"><span>熱門程度／排序</span><select name="sort" defaultValue={sort} onChange={submitOnChange}><option value="default">預設推薦</option><option value="latest">最新</option><option value="popular">最熱門</option><option value="beginner">最適合入門</option><option value="engagement">參與度分數</option></select></label>
      </div>
