...
export async function PATCH(request: Request) { try { const admin = await actor(request, ["owner", "admin"]); const input = settingsSchema.parse(await request.json()); return NextResponse.json({ settings: await getAdminRepository().updateSettings(input, admin) }); } catch (e) { return invalid(e); } }
