    ...
    <main className="page-shell detail-page">
      ...
      <article>
        <header className="detail-hero">
          ...
          <aside className="detail-facts"><dl><div><dt>頻道</dt><dd>{item.channelTitle ?? "未提供"}</dd></div><div><dt>參與度分數</dt><dd>{(item.engagementScore * 100).toFixed(1)}%</dd></div><div><dt>觀看次數</dt><dd>{item.viewCount.toLocaleString("zh-Hant")}</dd></div><div><dt>適合誰</dt><dd>{item.suitableFor}</dd></div></dl><a className="primary-button" href={item.sourceUrl} target="_blank" rel="noreferrer">前往 YouTube ↗</a></aside>
        </header>
